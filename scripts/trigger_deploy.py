...
def trigger_deploy():
    ...
    print(f"🚀 Triggering Dokploy deployment webhook...")
    ...
    for attempt in range(1, max_retries + 1):
        try:
            with urllib.request.urlopen(req, timeout=30) as response:
                ...
                print(f"✅ Dokploy webhook responded with status {status_code}: {body}")
                print("🎉 Deployment successfully dispatched to Dokploy!")
                return
        except urllib.error.HTTPError as e:
            error_body = e.read().decode("utf-8", errors="replace")
            print(f"⚠️ Attempt {attempt}/{max_retries}: HTTP Error {e.code}: {error_body}")
    ...
    print("❌ Failed to trigger Dokploy deployment after all attempts.")
    sys.exit(1)
