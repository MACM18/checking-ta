    ...
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            @if(Auth::user()->canEdit())
                <a href="{{ route('documents.create') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-lg font-semibold text-sm text-white tracking-wide hover:bg-indigo-700 active:bg-indigo-800 shadow-sm transition">
                                ...
                                <tr class="hover:bg-slate-50/80 transition">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-xs font-medium space-x-2">
                                        ...
                                        @if(Auth::user()->isAdmin() || $doc->created_by === Auth::id())
                                            <form action="{{ route('documents.destroy', $doc) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete document {{ $doc->document_number }}?');">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="inline-flex items-center px-2 py-1.5 rounded hover:bg-red-50 text-red-600 transition">
